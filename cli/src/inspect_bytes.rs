    ...
    while cursor < bytes.len() {
        ...
        match atom {
            ...
            Atom::Bytes(bytes) => {
                ...
                let num_payload_words = bytes.len().round_up_to_multiple_of(8) / 8;
                for i in 0..num_payload_words {
                    info.push(if i == num_payload_words - 1 {
                        WordInfo::BytesContinuation {
                            num_relevant: (bytes.len() % 8) as u8,
                            is_last: true,
                        }
                    } else {
                        WordInfo::BytesContinuation {
...
impl WordInfo {
    fn starts_atom(&self) -> bool {
        matches!(self, WordInfo::Block { .. } | WordInfo::Bytes { .. })
    }
    fn ends_atom(&self) -> bool {
        matches!(self, WordInfo::BytesContinuation { is_last, .. } if *is_last)
    }
}
